...
namespace mongols {
    ...
    bool http_request_parser::parse(const std::string& str) {
        ...
        this->settings.on_message_begin = http_request_parser::on_message_begin; /* [](http_parser * p) {
            return 0;
        };*/

        this->settings.on_header_field = http_request_parser::on_header_field; /* [](http_parser *p, const char *buf, size_t len) {
        ...
        this->settings.on_header_value = http_request_parser::on_header_value; /* [](http_parser *p, const char *buf, size_t len) {
            tmp_* THIS = (tmp_*) p->data;
            THIS->parser->req.headers[THIS->pair.first] = std::move(std::string(buf, len));
            return 0;
        };*/
        ...
        this->settings.on_status = http_request_parser::on_status; /* [](http_parser*, const char *at, size_t length) {
            return 0;
        };*/
        ...
        this->settings.on_headers_complete = http_request_parser::on_headers_complete; /* [](http_parser * p) {
            tmp_* THIS = (tmp_*) p->data;
            THIS->parser->req.method = std::move(http_method_str((enum http_method)p->method));
            struct http_parser_url u;
            http_parser_url_init(&u);
        ...
        this->settings.on_message_complete = http_request_parser::on_message_complete; /* [](http_parser * p) {
            return 0;
        };*/

        this->settings.on_chunk_header = http_request_parser::on_chunk_header; /* [](http_parser * p) {
            return 0;
        };*/

        this->settings.on_chunk_complete = http_request_parser::on_chunk_complete; /* [](http_parser * p) {
            return 0;
        };*/
