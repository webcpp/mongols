...
namespace mongols
{
    ...
    udp_server::udp_server(const std::string &host, int port, int timeout, size_t buffer_size, int max_event_size)
        : host(host), port(port), listenfd(0), max_event_size(max_event_size), server_is_ok(false), server_hints(), server_info(0), cleaning_fun(), whitelist_inotify(), server_epoll(0), buffer_size(buffer_size), timeout(timeout), clients(udp_server::clients_size), blacklist(udp_server::backlist_size), whitelist(), enable_blacklist(false), enable_whitelist(false), enable_security_check(false)
    {
    ...
    udp_server::client_t::client_t()
        : ip(), port(-1), t(time(0)), count(0)
    {
    }

    udp_server::client_t::client_t(const std::string &ip, int port)
        : ip(ip), port(port), t(time(0)), count(0)
    {
    }
    ...
    bool udp_server::check_blacklist(const std::string &ip)
    {
        ...
        if (this->blacklist.tryGet(ip, black_ip))
        {
            ...
            if ((diff == 0 && black_ip->count > udp_server::max_connection_limit) || (diff > 0 && black_ip->count / diff > udp_server::max_connection_limit))
            {
                black_ip->t = time(0);
    ...
    bool udp_server::security_check(const udp_server::client_t &client)
    {
        ...
        if ((diff == 0 && client.count > udp_server::max_send_limit) || (diff > 0 && client.count / diff > udp_server::max_send_limit))
        {
            return false;
        }
        return true;
