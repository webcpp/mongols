...
namespace mongols
{
    ...
    medis_server::medis_server(const std::string &host, int port, int timeout, size_t buffer_size, size_t thread_size, int max_event_size)
        : server(0), db(0), options(), sqldb(0), vm(), resp_decoder(), resp_encoder(), op(), lru_string_max_size(1024), lru_map_max_size(1024), lru_list_max_size(1024), lru_set_max_size(1024), lru_queue_max_size(1024), lru_stack_max_size(1024), string_data(), map_data(), list_data(), set_data(), queue_data(), stack_data()
    {
        if (thread_size > 0)
        {
    ...
    bool medis_server::set_openssl(const std::string &crt_file, const std::string &key_file, openssl::version_t v, const std::string &ciphers, long flags)
    {
    ...
    void medis_server::set_enable_blacklist(bool b)
    {
        this->server->set_enable_blacklist(b);
    }

    void medis_server::set_enable_whitelist(bool b)
    {
        this->server->set_enable_whitelist(b);
    }
    void medis_server::set_whitelist(const std::string &ip)
    {
    ...
    void medis_server::run(const std::string &path, const std::string &db_name)
    {
        ...
        if (leveldb::DB::Open(this->options, path, &this->db).ok())
        {
            ...
            try
            {
                ...
                this->server->run(std::bind(&medis_server::work, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));
            }
            catch (std::exception &)
            {
            }
    ...
    std::string medis_server::work(const std::pair<char *, size_t> &input, bool &keepalive, bool &send_to_other, tcp_server::client_t &client, tcp_server::filter_handler_function &f)
    {
        send_to_other = false;
    ...
    std::string medis_server::sdifference(const std::vector<std::string> &ret)
    {
        ...
        {
            std::string v1, v2;
            if (this->db->Get(leveldb::ReadOptions(), ret[1], &v1).ok() && this->db->Get(leveldb::ReadOptions(), ret[2], &v2).ok())
            {
                try
                {
                    mongols_set s1, s2;
    ...
    medis_server::cache_t<value_t>::cache_t()
        : data(), t(time(0)), expires(-1)
    {
    }

    template <class value_t>
    medis_server::cache_t<value_t>::cache_t(const value_t &v)
        : data(v), t(time(0)), expires(-1)
    {
    }
