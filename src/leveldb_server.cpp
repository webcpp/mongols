...
namespace mongols
{
    ...
    void leveldb_server::run(const std::string &path)
    {
        ...
        {
            this->server->run(std::bind(&leveldb_server::filter, this, std::placeholders::_1), std::bind(&leveldb_server::work, this, std::placeholders::_1, std::placeholders::_2));
        }
    }
    ...
    bool leveldb_server::set_openssl(const std::string &crt_file, const std::string &key_file, openssl::version_t v, const std::string &ciphers, long flags)
    {
