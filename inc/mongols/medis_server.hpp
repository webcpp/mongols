...
namespace mongols
{
    ...
    public:
        medis_server() = delete;
        medis_server(const std::string &host, int port, int timeout = 5000, size_t buffer_size = 8192, size_t thread_size = std::thread::hardware_concurrency(), int max_event_size = 64);
    ...
    private:
        ...
        std::string work(
            const std::pair<char *, size_t> &, bool &, bool &, tcp_server::client_t &, tcp_server::filter_handler_function &);
        ...
        size_t lru_string_max_size, lru_map_max_size, lru_list_max_size, lru_set_max_size, lru_queue_max_size, lru_stack_max_size;
